...
class IntegrateJson(pyblish.api.InstancePlugin):
    ...
    def process(self, instance):
        ...
        self.log.debug('png instance: {}'.format(instance))
        ...
        for repre in instance.data.get("representations"):
            ...
            with open(json_publish_path, "r+") as publish_json:
                ...
                published_data['project']['clip']['layers'].append(published_layer_data)
                publish_json.seek(0)  
                json.dump(published_data, publish_json, indent=4)
                publish_json.truncate()
            
            self.log.debug('Add layer_data to Json file: {}'.format(json_publish_path))
    ...
    def _get_json_repre(self, context):
        ...
        if len(json_repre) != 1:
            raise Exception('Exporting multiple json is not supported: {}'.format(json_repre)) 
