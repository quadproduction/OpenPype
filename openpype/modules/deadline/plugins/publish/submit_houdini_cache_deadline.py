...
class HoudiniCacheSubmitDeadline(abstract_submit_deadline.AbstractSubmitDeadline,   # noqa
    ...
    def get_job_info(self):
        ...
        start_frame = rop_node.evalParm("f1")
        end_frame = rop_node.evalParm("f2")
        if rop_node.type().name() != "alembic":
            frames = "{start}-{end}".format(start=int(start_frame),
                                            end=int(end_frame))
    ...
    def get_plugin_info(self):
        ...
        script = "{OPENPYPE_REPOS_ROOT}/openpype/scripts/remote_publish.py"
