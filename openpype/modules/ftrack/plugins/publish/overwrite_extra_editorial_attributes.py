...
class OverwriteExtraEditorialAttributes(pyblish.api.InstancePlugin):
    """Integrate extra editorial attributes."""

    order = pyblish.api.IntegratorOrder + 0.475
    label = "Overwrite Extra Editorial Attributes"
    ...
    def process(self, instance):
        if instance.data.get('is_custom_attrs_set'):
            self.log.warning("Custom attributes already set")
            return
        ...
        otio_attributes = self._get_otio_attributes(
            instance,
            ftrack_custom_attrs,
            rush_and_cut_data
        )
    ...
    def _get_otio_attributes(self, instance, custom_attrs, settings):
        old_frame_start = custom_attrs[settings['frame_start']]
        old_rush_frame_in = custom_attrs[settings['rush_frame_in']]
        ...
        frame_start = old_rush_frame_in - \
            instance.data['sourceStart'] + old_frame_start

        frame_end = (rush_frame_out - rush_frame_in + 1) + \
            (old_rush_frame_in - rush_frame_in + old_frame_start)
