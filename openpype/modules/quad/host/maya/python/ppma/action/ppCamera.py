...
logger = logging.getLogger('ppma.action.ppCamera')
...
class Camera(object):
	...
	def _get_ctrl_name(self, name):
		...
		"""
		if self.namespace:
			return "{namespace}:{ctrl}".format(namespace=self.namespace, ctrl=self.ctrl.get(name))
		else:
	...
	def push_camera_position_to_rig(self):
		...
		"""
		# get camera position
		world_translation = cmds.xform(self.camera, query=True, worldSpace=True, translation=True)
		world_rotation = cmds.xform(self.camera, query=True, worldSpace=True, rotation=True)

		# set world translation to triangle_ctrl
		if cmds.objExists(self._get_ctrl_name("triangle")):
			# set attr
			cmds.xform(self._get_ctrl_name("triangle"), worldSpace=True, translation=world_translation)
			cmds.setAttr("{n}.rotate".format(n=self._get_ctrl_name("triangle")), 0, 0, 0)

		# set world rotation Y to circle
		if cmds.objExists(self._get_ctrl_name("circle")):
			# set attr
			rotation = [0, world_rotation[1], 0]
			cmds.xform(self._get_ctrl_name("circle"), worldSpace=True, translation=world_translation)
			cmds.setAttr("{n}.rotate".format(n=self._get_ctrl_name("circle")), rotation[0], rotation[1], rotation[2])

		# set world rotation X to square
		if cmds.objExists(self._get_ctrl_name("square")):
			# set attr
			rotation = [world_rotation[0], 0, 0]
			cmds.xform(self._get_ctrl_name("square"), worldSpace=True, translation=world_translation)
			cmds.setAttr("{n}.rotate".format(n=self._get_ctrl_name("square")), rotation[0], rotation[1], rotation[2])

		# set world rotation Z to cross
		if cmds.objExists(self._get_ctrl_name("cross")):
			# set attr
			rotation = [0, 0, world_rotation[2]]
			cmds.xform(self._get_ctrl_name("cross"), worldSpace=True, translation=world_translation)
			cmds.setAttr("{n}.rotate".format(n=self._get_ctrl_name("cross")), rotation[0], rotation[1], rotation[2])

		# set target
		if cmds.objExists(self._get_ctrl_name("target")):
			# set attr translation same as camera
			cmds.xform(self._get_ctrl_name("target"), worldSpace=True, translation=world_translation)
			# set attr translateZ -10
			cmds.setAttr("{n}.translateZ".format(n=self._get_ctrl_name("target")), cmds.getAttr("{n}.translateZ".format(n=self._get_ctrl_name("target")))-10)
...
def push_camera_position_to_rig_selection():
	...
	if sels:
		return push_camera_position_to_rig(camera=sels[0])
...
def push_camera_position_to_rig(camera):
	...
	if is_a_camera(camera):
		...
		ppUi.message_dialog(title="push_camera_position_to_rig", message="Le node n'est pas une camera.", message_type="information", button="Desole", cancel_button=None)
