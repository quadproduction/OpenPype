...
class PPCreateToolsetsPanel(nukescripts.PythonPanel):
    def __init__(self):
        ...
        self.locationButton = nuke.Enumeration_Knob('location', 'Location', ['Local', 'Shared'])

        # ADD KNOBS
        self.addKnob(self.menuItemChoice)
        self.addKnob(self.menuPath)
        self.addKnob(self.locationButton)
    ...
    def createPreset(self):
        path = None
        print str(self.menuPath.value())
        if self.locationButton.value() == 'Shared':
            path = os.environ['PP_USER_NUKE_COMMON_PATH']
        if nuke.createToolset(filename=str(self.menuPath.value()), rootPath=path):
...
def addToolsetsPanel():
    res = False
    if nuke.nodesSelected() is True:
        res = PPCreateToolsetsPanel().showModalDialog()
...
def refreshToolsetsMenu():
    ...
    m = toolbar.findItem("Fix ToolSet")
    print m
    if m is not None:
        m.clearMenu()
...
def createToolsetsMenu(toolbar):
    m = toolbar.addMenu("Fix ToolSet", "pp_toolsets.png")
    m.addCommand("Create", "ppnk.tools.ppToolSet.addToolsetsPanel()", "", icon="ToolsetCreate.png")
    m.addCommand("-", "", "")
        ...
        for group in filecontents:
            ...
            if not os.path.isdir(fullFileName):
                ...
                if extPos != -1 and extPos == len(group) - 3:
                    ...
                    if delete:
                        m.addCommand(group, 'ppnk.tools.ppToolSet.deleteToolset("%s", "%s")' % (rootPath, fullFileName), "")
                        retval = True
                    else:
